...
export default function StudentDashboard() {
  ...
  const student = {
    name: 'Amara Okafor',
    studentId: 'SC-2024-8812',
    department: 'Computer Science',
    section: 'A',
    seat: 'A-112',
    avatar: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBYAc6o6BDMkZSnKmxeX85Wbgs-Iz1YQx13JyOtaiY_H81GA4afPwayVU5NGpwFxcnO4YvwyAOAUN-fs8mB2g5swse5ga47YnSIRTFOk7M201K5oE3W8QadwVsRFFncddiKOPETA335HK48lt-EkJOie-Plf7ppEtvULy1cSRJ4pnuLLr8MgGb8GpFLKCad70lVQ-cQVUkb8r3zAeyrUodCd-zf_qHPARRFpVOISQTVcp8mG7RW-C4jt5c3ssmi4m_Hq7OKLEgulEU',
    qrCode: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDKZ3kEut8FlyMuaeqLXcjVu931e-BiFS3B7243Aer8mJjGnnQAh5DQKxVvdR6Oy_B_78XGKdzz73XC0stJV4gbbrPkgGjSCRluQ-ocYRyT1MueyWNm7JUaQp7JwndaoXNfTuzwoC6l-uhjKtcWdIkna97Vd1B_WqvnFAtaeZQYMvAGD2ckr36iRdW6FZYYpgFbU3EjH_8x_ChMjpWJyh6tKFO1gfw3s7bOjmboLq-5WvIlKYyhYOfKclM33PPw_CvUK31lW_JjbTA',
  };

  const stages = [
    { label: 'Registered', icon: 'check', completed: true, active: false },
    { label: 'Checked-in', icon: 'check', completed: true, active: false },
    { label: 'Seated', icon: 'check', completed: true, active: false },
    { label: 'Gown Issued', icon: 'inventory_2', completed: true, active: true },
    { label: 'Completed', icon: 'school', completed: false, active: false },
  ];
    ...
    <div className="flex flex-col min-h-screen bg-surface">
      ...
      <header className="sticky top-0 w-full z-40 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md shadow-sm dark:shadow-none">
        <div className="flex items-center justify-between px-4 md:px-6 py-4 w-full max-w-6xl mx-auto">
          <div className="flex items-center gap-2 md:gap-3">
            <span className="material-symbols-outlined text-blue-900 dark:text-blue-200 text-base md:text-lg">menu</span>
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-4 md:px-6 pt-4 md:pt-8 pb-28 md:pb-8 space-y-6 md:space-y-8">
        ...
        <section className="bg-surface-container-lowest rounded-lg md:rounded-xl p-6 md:p-8 flex flex-col items-center shadow-sm relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full" style={{ background: 'linear-gradient(135deg, #002547 0%, #1b3b5f 100%)' }}></div>
                ...
                <div key={index} className="flex flex-col items-center space-y-1 md:space-y-2">
                  <div 
                    className={`w-5 md:w-6 h-5 md:h-6 rounded-full flex items-center justify-center z-10 transition-all ${
                      stage.active
                        ? 'ring-2 md:ring-4 ring-primary-fixed'
                        : ''
                    }`}
                    style={{
                      background: stage.completed || stage.active 
                  ...
                  >
                    <span 
                      className={`material-symbols-outlined text-xs md:text-sm ${
                        stage.completed || stage.active ? 'text-white' : 'text-on-surface'
                      }`}
