          ...
          >
            <div className="flex justify-between items-start gap-1.5 xs:gap-2 mb-1.5 xs:mb-2">
              <div className="flex-1 min-w-0">
                <p className="text-[9px] xs:text-[10px] text-on-surface-variant font-label mb-0.5 xs:mb-1">
                  {row.time}
                </p>
              ...
              <span
                className={`px-1.5 xs:px-2 py-0.5 xs:py-1 rounded-full text-[8px] xs:text-[9px] font-bold uppercase whitespace-nowrap flex-shrink-0 ${getStageColor(row.stageLabel)}`}
              >
                {row.stageLabel.split(" ")[0]}
              </span>
            </div>
            <p className="text-[8px] xs:text-[9px] text-on-surface-variant truncate">
              {row.location}
            </p>
          </div>
        ))}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-surface-container-low text-[9px] xs:text-[10px] uppercase tracking-widest text-on-surface-variant font-label">
              ...
              <th className="px-2 xs:px-3 md:px-6 py-2 xs:py-3 hidden sm:table-cell">
                Name
              </th>
              <th className="px-2 xs:px-3 md:px-6 py-2 xs:py-3">Stage</th>
              <th className="px-2 xs:px-3 md:px-6 py-2 xs:py-3 text-right hidden lg:table-cell">
                Location
              </th>
            </tr>
          </thead>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <td className="px-2 xs:px-3 md:px-6 py-2 xs:py-3">
                  <span
                    className={`px-1.5 xs:px-2 py-0.5 rounded-full text-[8px] xs:text-[9px] font-bold uppercase whitespace-nowrap ${getStageColor(row.stageLabel)}`}
                  >
                    {row.stageLabel}
                  </span>
                </td>
                <td className="px-2 xs:px-3 md:px-6 py-2 xs:py-3 text-right text-on-surface-variant hidden lg:table-cell text-xs sm:text-sm">
                  {row.location}
                </td>
              </tr>
            ))}
