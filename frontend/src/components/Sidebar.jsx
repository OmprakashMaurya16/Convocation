...
export default function Sidebar({ activeItem, setActiveItem, open = false, setOpen = () => {} }) {
  const menuItems = [
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`w-56 sm:w-64 bg-surface-container-low flex flex-col border-r border-outline-variant/20 fixed h-screen overflow-y-auto z-40 transition-transform duration-300 md:translate-x-0 ${
      ...
      <div className="p-4 sm:p-6 flex items-center gap-2 sm:gap-3">
        ...
        <h1 className="font-headline font-bold text-xs sm:text-lg text-primary leading-tight truncate">Convocation Ledger</h1>
      </div>
      ...
      <nav className="flex-1 px-4 space-y-1">
        {menuItems.map((item) => (
          ...
          >
            ...
            <span className="font-label font-semibold text-sm">{item.label}</span>
          </button>
        ))}
      </nav>
      ...
      <div className="p-6 border-t border-outline-variant/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-on-surface font-label">Admin Console</p>
            <p className="text-[10px] text-on-surface-variant uppercase tracking-widest font-label">Super User</p>
          </div>
        </div>
      </div>
      </aside>
