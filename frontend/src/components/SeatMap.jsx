...
export default function SeatMap() {
  ...
  return (
    <div className="bg-surface-container-lowest rounded-lg md:rounded-xl lg:rounded-2xl p-3 xs:p-4 md:p-6 lg:p-8 shadow-[0_8px_32px_rgba(0,31,42,0.02)] border border-outline-variant/10">
      {/* Legend */}
      <div className="flex items-center gap-1.5 xs:gap-2 md:gap-4 lg:gap-6 mb-4 xs:mb-6 md:mb-8 flex-wrap">
        <div className="flex items-center gap-1 xs:gap-2">
          ...
          <span className="text-[7px] xs:text-[8px] sm:text-xs md:text-sm font-medium text-slate-600 whitespace-nowrap">Occupied</span>
        </div>
        <div className="flex items-center gap-1 xs:gap-2">
          <span className="w-2 xs:w-2.5 h-2 xs:h-2.5 rounded-full bg-primary-container flex-shrink-0"></span>
          <span className="text-[7px] xs:text-[8px] sm:text-xs md:text-sm font-medium text-slate-600 whitespace-nowrap">Reserved</span>
        </div>
        <div className="flex items-center gap-1 xs:gap-2">
          ...
          <span className="text-[7px] xs:text-[8px] sm:text-xs md:text-sm font-medium text-slate-600 whitespace-nowrap">Empty</span>
        </div>
        <div className="flex items-center gap-1 xs:gap-2">
          <span className="w-2 xs:w-2.5 h-2 xs:h-2.5 rounded-full bg-error flex-shrink-0"></span>
          <span className="text-[7px] xs:text-[8px] sm:text-xs md:text-sm font-medium text-slate-600 whitespace-nowrap hidden xs:inline">Flag</span>
        </div>
      ...
      {/* Seat Grid Container */}
      <div className="relative bg-surface rounded-lg md:rounded-xl p-3 xs:p-4 md:p-6 lg:p-10 min-h-[300px] xs:min-h-[350px] md:min-h-[450px] lg:min-h-[500px] flex flex-col items-center overflow-x-auto">
        {/* Stage */}
        <div className="w-full md:w-3/4 h-8 xs:h-10 md:h-12 bg-primary-container rounded-b-2xl md:rounded-b-3xl mb-4 xs:mb-6 md:mb-8 lg:mb-16 flex items-center justify-center shadow-lg flex-shrink-0">
          <span className="text-[8px] xs:text-[9px] sm:text-xs md:text-sm font-headline font-extrabold text-white tracking-[0.2em] md:tracking-[0.3em] uppercase line-clamp-1">Stage</span>
        </div>

        {/* Seat Rows */}
        <div className="flex flex-col gap-1 xs:gap-1.5 md:gap-3 lg:gap-4">
          {['A', 'B', 'C'].map((rowLabel) => (
            <div key={rowLabel} className="flex gap-1 xs:gap-1.5 md:gap-2 lg:gap-3 justify-center flex-wrap">
              {seatRows[rowLabel].map((seat, idx) => (
                seat ? (
                  ...
                  <div key={`aisle-${rowLabel}-${idx}`} className="w-2 xs:w-3 md:w-6 lg:w-12"></div>
                )
              ))}
            </div>
          ))}
        ...
        <div className="absolute bottom-4 xs:bottom-6 md:bottom-8 right-4 xs:right-6 md:right-8 text-on-surface opacity-5 select-none pointer-events-none hidden md:block">
          <span className="material-symbols-outlined text-6xl md:text-7xl lg:text-8xl">architecture</span>
        </div>
      </div>
      ...
      {selectedSeat && (
        <div className="fixed bottom-20 xs:bottom-24 md:bottom-8 left-4 right-4 xs:left-auto xs:right-auto md:absolute md:bottom-8 md:left-1/2 md:-translate-x-1/2 bg-primary-container text-white px-3 xs:px-4 md:px-6 py-3 xs:py-4 rounded-lg md:rounded-2xl shadow-2xl flex flex-col md:flex-row items-center gap-2 xs:gap-3 md:gap-6 border border-white/10 backdrop-blur-xl z-10 touch-none">
          <div className="flex flex-col text-center md:text-left">
            <span className="text-[8px] xs:text-[9px] md:text-[10px] font-bold text-on-primary-container uppercase tracking-widest">Selected</span>
            <span className="text-xs xs:text-sm md:text-base font-bold">{selectedSeat}</span>
          </div>
          ...
          <div className="flex gap-1 xs:gap-2 md:gap-3 w-full md:w-auto">
            <button className="flex-1 md:flex-initial flex items-center justify-center gap-1 xs:gap-2 text-[8px] xs:text-[9px] md:text-xs font-bold px-2 xs:px-3 md:px-4 py-1.5 xs:py-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors whitespace-nowrap touch-none min-h-[44px] md:min-h-auto">
              <span className="material-symbols-outlined text-xs md:text-sm">swap_horiz</span>
